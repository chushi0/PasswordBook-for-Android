...
namespace libsha {
    ...
    int ztransform(const uint8_t *msg, uint32_t *h) {
        uint32_t w[64];
    ...
    void writeInt(uint32_t v, char *out) {
        out[0] = (v >> 24) & 0xff;
        out[1] = (v >> 16) & 0xff;
        out[2] = (v >> 8) & 0xff;
        out[3] = v & 0xff;
    ...
    void sha256(const char *input, int len, char *out) {
        uint32_t hash[8];
        zsha256(reinterpret_cast<const uint8_t *>(input), len, hash);
        writeInt(hash[0], out);
        writeInt(hash[1], out + 4);
        writeInt(hash[2], out + 8);
        writeInt(hash[3], out + 12);
        writeInt(hash[4], out + 16);
        writeInt(hash[5], out + 20);
        writeInt(hash[6], out + 24);
        writeInt(hash[7], out + 28);
    }
}